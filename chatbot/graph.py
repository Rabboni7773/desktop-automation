...
from chatbot.prompts import SYSTEM_PROMPT_SUMMARY, SYSTEM_PROMPT_NOT_SUMMARY, CHATBOT_SYS_PROMPT
...
async def message_summarizer(state: ChatState):
    ...
    if current_summary:
        prompt = [
            ...
            HumanMessage(content=f"Summary : {current_summary} \n Extra Messages : {extra_msgs}"),
        ]
    else:
        prompt = [
            SystemMessage(content=SYSTEM_PROMPT_NOT_SUMMARY),
            HumanMessage(content=f"Extra Messages : {extra_msgs}"),
        ]

    summary_generated = await summary_model.ainvoke(prompt)
    return {"summary": summary_generated.content, "messages": [RemoveMessage(id=msg.id) for msg in extra_msgs]}


async def chat_node(state: ChatState):
    sys_prompt = [SystemMessage(content=CHATBOT_SYS_PROMPT)]
    summary_message = SystemMessage(content=state.get("summary", " "))
    response = await model.ainvoke(sys_prompt + [summary_message] + state["messages"])
